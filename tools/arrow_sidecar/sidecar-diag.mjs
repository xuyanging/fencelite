...
var CALLOUT_DETECTION_VERSION = "callout-vector-v18";
...
var leaderCandidates = (scene, pageDiagonal, segmentation, carrierMembers = /* @__PURE__ */ new Set()) => {
  ...
  for (let opIndex = 0; opIndex < scene.ops.length; opIndex += 1) {
    ...
    const previousMarkerAttached = isCompactArrowhead(previous, pageDiagonal) && Math.min(...endpoints.map((endpoint) => pointToBounds(endpoint, finiteBounds(previous.bounds)))) <= Math.max(
      pageDiagonal * 35e-5,
      boundsDiagonal(previous.bounds) * 0.25
    );
...
var expandOutlineLeader = (scene, leader, cluster, arrowheads, outlineMembers, segmentation, pageDiagonal) => {
  ...
  const nearestMarker = (point) => {
    const ranked = [...markerPool].map(([opIndex, op]) => ({
      opIndex,
      distance: pointToBounds(point, finiteBounds(op.bounds))
    })).filter((candidate) => candidate.distance <= markerThreshold).sort((left, right) => left.distance - right.distance || left.opIndex - right.opIndex);
    return ranked[0];
  };
  const directMarker = nearestMarker(target);
  ...
  for (let opIndex = searchStart; opIndex <= searchEnd; opIndex += 1) {
    ...
    const parallel = Math.abs(currentDx * extensionDx + currentDy * extensionDy) / (currentLength * extensionLength);
    ...
    if (connectionGap > (parallel >= 0.985 ? collinearConnection : normalConnection)) continue;
    if (pointToBounds(other, cluster.bounds) < cluster.scale * 2.5) continue;
    const marker = nearestMarker(other);
    if (!marker) continue;
    const score = connectionGap + marker.distance + Math.abs(opIndex - leader.opIndex) * 0.1 + Math.abs(groupForOp(segmentation, opIndex) - leaderGroup) * 0.1;
    extensionCandidates.push({ opIndex, other, markerOp: marker.opIndex, score });
  }
  extensionCandidates.sort((left, right) => left.score - right.score);
  const extension = extensionCandidates[0];
  const directIsAtEndpoint = Boolean(directMarker && directMarker.distance <= Math.max(cluster.scale * 0.25, pageDiagonal * 5e-4));
...
function detectCalloutsInternal(scene, segmentation, partialOptions = {}, forcedOutlineTextRois = []) {
  ...
  const rawProposals = leaders.flatMap((leader) => textClusters.flatMap((cluster) => {
    ...
    const geometricLeaderEscape = symbolCarrier ? targetDistance >= Math.max(cluster.scale * 0.45, pageDiagonal * 12e-4) : targetDistance >= rootThreshold * 0.65;
  ...
  const proposals = rawProposals.filter((proposal) => proposal.qualifies).sort((left, right) => proposalScore(right) - proposalScore(left));
  ...
  for (const [cluster, owned] of leadersByCluster) {
    ...
    while (addedBranch && owned.length < 4) {
      ...
      const candidates = leaders.flatMap((leader) => {
        ...
        const candidateStart = candidatePacket[0];
        const candidateEnd = candidatePacket.at(-1);
        const ownerStart = ownerPacket[0];
        const ownerEnd = ownerPacket.at(-1);
        const packetGap = candidateEnd < ownerStart ? ownerStart - candidateEnd - 1 : ownerEnd < candidateStart ? candidateStart - ownerEnd - 1 : 0;
        if (leaderGroup === ownerGroup ? packetGap > 1 : connection.distance > exactConnectionThreshold) return [];
        const arrowAtFreeEnd = expandedLeader.arrowheadOps.some((opIndex) => {
          const op = scene.ops[opIndex];
          return op?.kind === "path" && pointToBounds(freeEnd.point, finiteBounds(op.bounds)) <= connectionThreshold * 2;
        });
